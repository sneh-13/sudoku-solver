...
int checkPuzzle(Square *** sudoku, Box ** boxes)
{
    ...
    // First, check for squares with a single possibility.
    for (i = 0; i < SIZE_ROWS; i++) {
        for (j = 0; j < SIZE_COLUMNS; j++) {
    ...
    // Try to solve by checking rows using the rowSingles technique.
    if (rowSingles(sudoku))
        return 1;
    
    // Fallback: try solving by checking box singles.
